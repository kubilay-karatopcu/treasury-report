#!/usr/bin/env python3
"""Faz A2 ekstraksiyonu: kaynak app.py -> mevduat_panel Outstanding üçlüsü.
...
- @app.route -> @mevduat_panel_bp.route + @login_required
...
for r in (rc, ro):
    pass
rc = rc.replace('@app.route("', '@mevduat_panel_bp.route("')
ro = ro.replace('@app.route("', '@mevduat_panel_bp.route("')
rc = rc.replace('methods=["GET"])\ndef ', 'methods=["GET"])\n@login_required\ndef ')
ro = ro.replace('methods=["GET"])\ndef ', 'methods=["GET"])\n@login_required\ndef ')

# ── yaz ────────────────────────────────────────────────────────────────────
(MOD / "engine").mkdir(exist_ok=True)
(MOD / "engine" / "__init__.py").write_text(HDR % "mevduat_panel hesap motorları.")
outputs = {
