#!/usr/bin/env python3
"""Faz A0 donusum scripti: NIM_calculation index.html -> nim_panel modulu.
...
s = s[:i] + '  <link rel="stylesheet" href="{{ url_for(\'nim_panel.static\', filename=\'nim_panel.css\') }}?v={{ nim_version }}">' + s[j:]
...
inject = (
    '<script>\n'
    '  // Port bootstrap konfigurasyonu — nim_panel.js bunu okur.\n'
    '  window.NIM_CONFIG = {\n'
    '    apiBase: {{ url_for("nim_panel.index") | tojson }},\n'
    '    masaUrl: {{ masa_url | tojson }}\n'
    '  };\n'
    '</script>\n'
    '<script src="{{ url_for(\'nim_panel.static\', filename=\'nim_panel.js\') }}?v={{ nim_version }}"></script>'
)
s = s[:i] + inject + s[j:]

# ── 3. CDN -> vendor static ────────────────────────────────────────────────
V = "{{ url_for('nim_panel.static', filename='vendor/%s') }}"
cdn_map = {
...
css += """
/* ═══ Port eki (nim_panel): Masa'ya donus linki — PRISMA kabuguna kopru ═══ */
.masa-link {
  display: block;
  padding: 0 20px 10px;
  font-size: 11px;
...
# ── 8. Yaz ─────────────────────────────────────────────────────────────────
(MOD / "templates" / "nim_panel").mkdir(parents=True, exist_ok=True)
(MOD / "static").mkdir(parents=True, exist_ok=True)
(MOD / "templates" / "nim_panel" / "index.html").write_text(s)
(MOD / "static" / "nim_panel.css").write_text(css)
(MOD / "static" / "nim_panel.js").write_text(js)
