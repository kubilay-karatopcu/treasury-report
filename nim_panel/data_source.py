...
SQL dosyaları `nim_panel/queries/*.sql` — kaynak reponun `queries/prod/`
...
def load_dataframe(name: str, params: dict | None = None) -> pd.DataFrame:
    """`nim_panel/queries/{name}.sql`'i DataClient havuzu üzerinden koşar.
    ...
    if dc is None or not hasattr(dc, "edw_query_to_pandas"):
        raise RuntimeError(
            "nim_panel prod DataClient gerektirir (DEV stub'ında Oracle yolu yok; "
    ...
    try:
        ...
        try:
            ...
            current_app.logger.exception("nim_panel: havuz bağlantısı bırakılamadı")
