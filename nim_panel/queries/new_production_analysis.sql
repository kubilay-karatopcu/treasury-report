WITH base AS (
    ...
    -- Sabit başlangıç (eski: son 6 ay). Gün-gün outstanding üretimi (Sector
    -- Comparison) 2025-10'dan itibaren TAM kitap gerektirir → akım geçmişi
    -- 2025-01'e kadar geri gider ki o tarihte yaşayan vadeli mevduatlar
    -- (9 aya kadar geriden gelen valörler) veride bulunsun.
    WHERE VAL_DT >= TO_DATE('01/01/2025', 'dd/mm/yyyy')
)
SELECT
    VAL_DT,
    CCY_CODE,
    ...
    SUM(NVL(YENI_PARA, 0))                                              AS YENI_PARA_TOPLAM,
    SUM(MAX_AUM)                                                        AS MAX_AUM_TOPLAM,
    SUM(TOTAL_AUM)                                                      AS TOTAL_AUM_TOPLAM,
    SUM(TRY_BALANCE * EKSTREM)          / NULLIF(SUM(TRY_BALANCE), 0)   AS WAVG_EKSTREM,
    SUM(TRY_BALANCE * TCMB_WEEKLY_RATE) / NULLIF(SUM(TRY_BALANCE), 0)   AS WAVG_TCMB_WEEKLY_RATE,
    SUM(TRY_BALANCE * DTM)                                              AS TRY_X_DTM,
    SUM(TRY_BALANCE * INTRST_RT)                                        AS TRY_X_INTRST,
    SUM(TRY_BALANCE * EKSTREM)                                          AS TRY_X_EKSTREM,
    SUM(TRY_BALANCE * TCMB_WEEKLY_RATE)                                 AS TRY_X_TCMB
FROM base
GROUP BY
    VAL_DT, CCY_CODE, CUST_TP, RELATED_PC_CODE, KAMPANYA_ADI,
