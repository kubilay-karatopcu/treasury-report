...
> - **3.4 backend ✅ (C3)** — `routes_scope._scope_lineage_steps` (yan etkisiz lineage walker: türetme DAG'ını leaf→root yürür, her node için compile_*/compose_cached_sql ile adım SQL'i; çevrim/eksik-kaynak güvenli) + `GET /<pid>/scope/steps?alias=` endpoint (read-only, sqlparse pretty). 3 test. **KALAN: 3.4 frontend** — "Show steps" expandable panel (öneri: Hazırlık preview drawer'a "Adımlar" sekmesi; JSX + bundle build + cache-bust gerekir). UI yerleşim kararı kullanıcıya.
