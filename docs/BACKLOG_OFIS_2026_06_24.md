...
> - **2.4 ✅** — `DataClient.get_data`'ya opsiyonel `cancel_token`: açılan conn token'a `bind` → iptalde `conn.cancel()` BLOKLU in-flight sorguyu keser; iptal-kaynaklı oracledb hatası `BuildCancelled` olarak yükselir (failed değil); HER yolda `finally: conn.close()` (önceki leak fix). Token duck-typed → DataClient `presentations` import etmez. fetch.py 3 dc.get_data sitesine (`_pull`, Pass-2 filter, `_pull_source_into_duck`) token geçirildi. 2 test (her zaman close; pre-cancel→BuildCancelled+close).
> - **2.3 ✅** — `run_python_transform` `subprocess.run(timeout)`→`Popen`+poll (cancel VEYA wall_timeout→`proc.kill()`); stdout PIPE yerine dosyaya (pipe-buffer kilidi yok); iptalde `BuildCancelled`. Pass-2 python çağrısına token geçirildi. Cancel testi (koşan script <10s'de erken öldü, 30s wall beklenmedi); 13 executor testi geçer.
> - **2.6 ✅** — Frontend (hazirlik/index.jsx): `buildCancelUrl`; `buildJobRef` aktif job_id; build overlay'e **İptal** butonu (`abortActiveBuild`→`POST build-cancel`); `_pollBuild` `phase=cancelled/gone` ele alır + guard (iptal sonrası poll durur) + done/fail'de ref temizler; **beforeunload/unmount→`navigator.sendBeacon(build-cancel)`** (F5/sekme-kapatma sunucuya "dur" → orphan worker yok = B2 "F5 arkada çekiyor" biter). hazirlik.bundle.js cache-bust v39→40. DEV doğrulandı: build-cancel endpoint 200, Hazırlık bundle v40 render, konsol temiz. (Mevcut `cancelBuild` onay-modali ile çakışmadı — `abortActiveBuild` adı.) bundle gitignore → ofis `bash build.sh`.
> - **OTURUM 2 ÇEKİRDEK TAMAM (B1–B4):** 2.1+2.2+2.5 (main'de) + 2.4+2.3+2.6 (branch). Kalan opsiyonel: 2.7 (editör loading timeout — savunma; lock fix sonrası kritik değil).
